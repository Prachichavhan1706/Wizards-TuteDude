    ...
    switch (req.method) {
      ...
      case 'GET': {
        ...
        if (id) {
          ...
          return res.status(400).json({ error: 'Provide "id" or "itemId" query parameter.' });
        }
      }
      ...
      case 'PUT': {
        ...
        const { id } = req.query;
        if (!id) {
          return res.status(400).json({ error: 'Query parameter "id" is required for update.' });
        }
      ...
      case 'DELETE': {
        ...
        const { id } = req.query;
        if (!id) {
          return res.status(400).json({ error: 'Query parameter "id" is required for delete.' });
        }
